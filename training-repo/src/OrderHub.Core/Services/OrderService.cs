...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        foreach (var line in lines)
        {
            ...
            // Snapshot list price for all tiers; membership discount is applied once in CalculateTotal.
    ...
    public async Task<ServiceResult<Order>> CancelOrderAsync(int id)
    {
        ...
        // Restore stock before flipping status (previous code set Cancelled first, so restore never ran).
